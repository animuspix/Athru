...
float3 PRayDir(uint2 pixID,
               float pixWidth)
{
    float2 viewSizes = float2(DISPLAY_WIDTH,
                              DISPLAY_HEIGHT);
    viewSizes *= pixWidth;
    float2 xy = pixID - (viewSizes / 2.0f);
    float z = viewSizes.y / tan(FOV_RADS / 2.0f);
              ...
              float3 lensNormal*/)
{
    ...
    float pixWidth = sqrt(NUM_AA_SAMPLES);
    float2 viewSizes = float2(DISPLAY_WIDTH,
                              DISPLAY_HEIGHT) * pixWidth;
    incidentDir /= incidentDir.z / (viewSizes.y / tan(FOV_RADS / 2.0f));
