...
Engine::Engine()
{
	// Start input and graphics pointers at [nullptr]
	m_pInput = nullptr;
	m_pGraphics = nullptr;
	...
	if (m_pInput == false)
	{
		// Abandon ship if input fails to load
		return;
	}
	...
	if (m_pGraphics == false)
	{
		// Abandon ship if graphics fails to load
		return;
	}
}
...
void Engine::BuildWindow(int& screenWidth, int& screenHeight)
{
	WNDCLASSEX wc;
...
Engine::~Engine()
{
	// Delete the graphics manager, then send it to [nullptr]
	if (m_pGraphics)
	{
		delete m_pGraphics;
		m_pGraphics = nullptr;
	...
	// Delete the input manager, then send it to [nullptr]
	if (m_pInput)
	{
		delete m_pInput;
		m_pInput = nullptr;
